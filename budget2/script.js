...
const defaultData = {
    monthly: [
        ...
        { name: '401k', formula: 'Five9 * 0.04 * -1', category: 'Required' },
        { name: 'Roth', formula: '-617.50', category: 'Required' },
        { name: 'Taxes', formula: '-650', category: 'Required' },
    ...
    portfolio: [
        { stock: 'AAPL', shares: 1.7181, purchasePrice: 290.82, currentPrice: 333.74 },
        ...
        { stock: 'VOO', shares: 14.5556, purchasePrice: 687.00, currentPrice: 683.17 },
        { stock: 'AAPL', shares: 1.0280, purchasePrice: 277.00, currentPrice: 333.74 },
        { stock: 'GOOGL', shares: 0.4153, purchasePrice: 361.21, currentPrice: 346.77 },
        { stock: 'GOOGL', shares: 0.0799, purchasePrice: 361.26, currentPrice: 346.77 },
        { stock: 'NVDA', shares: 1.3980, purchasePrice: 203.97, currentPrice: 202.81 }
    ]
};
...
function normalizeBudgetData() {
    ...
    if (!Array.isArray(budgetData.monthly)) {
        ...
        budgetData.monthly = budgetData.monthly.map(entry => ({
            name: entry.name,
            formula: entry.formula != null ? String(entry.formula) : String(entry.amount ?? 0),
...
function evaluateMonthlyFormula(formula, stack = []) {
    ...
    const expanded = expression.replace(/value\(\s*(['"])(.*?)\1\s*\)/g, (match, quote, key) => {
        const value = getMonthlyValue(key, stack);
        return `(${value})`;
    });

    const safeExpression = expanded.replace(/\b[A-Za-z_][A-Za-z0-9_]*\b/g, token => {
...
function calculateGainLoss(shares, purchasePrice, currentPrice) {
    ...
    const percent = purchasePrice === 0 ? 0 : ((currentPrice - purchasePrice) / purchasePrice) * 100;
    return { gain, percent };
}
